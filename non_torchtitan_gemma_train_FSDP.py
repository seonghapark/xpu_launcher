#!/usr/bin/env python3
"""FSDP2 Gemma training smoke test for XPU.

This is copied from the non-TorchTitan trainer and adds PyTorch FSDP2 sharding
for long sequence training across ranks. It keeps the same PG19/MultiNews data
path and supports either lm_head-only or full-model training.
"""
...
def apply_fsdp2_sharding(model: Any, torch: Any, dist_info: dict[str, Any], args: argparse.Namespace) -> bool:
    ...
    if decoder_layers is None:
        raise RuntimeError("expected Gemma-style model.model.layers for FSDP2 sharding")
