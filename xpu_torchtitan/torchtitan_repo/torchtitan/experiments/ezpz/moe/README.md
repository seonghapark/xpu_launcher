...
MODULE=ezpz.moe CONFIG=moe_debugmodel bash torchtitan/experiments/ezpz/run_train.sh --debug.moe-force-load-balance
```

```bash
# 16B parameter model: adapted from older 16B parameter model from https://huggingface.co/deepseek-ai/deepseek-moe-16b-base
MODULE=ezpz.moe CONFIG=moe_16b bash torchtitan/experiments/ezpz/run_train.sh
```

```bash
# 671B parameter model
MODULE=ezpz.moe CONFIG=moe_671b bash torchtitan/experiments/ezpz/run_train.sh
```
