...
# scripts/failover_lib.sh). ezpz splits the PBS allocation into active +
# spare, runs the inner command, scrapes bad-node signatures on any
...
# Optional: MAX_FAILOVER_RETRIES (default unbounded), IDLE_TIMEOUT (1800),
#           DATASET (blendcorpus | <hf-org>/<repo>), and the training vars
#           (LBS, GAS, OPTIMIZER, LR, CKPT_DIR, DFL_NAME, ...).
...
# Module loads are handled by ezpz_load_modules below (machine-aware).
...
# ---- Source ezpz-utils ----
# PBS copies the submit script into /var/spool/pbs/mom_priv/jobs/, so find
...
# it only splits the nodelist + runs the retry loop. So we still yeet
# .venv.tar.gz to the FULL allocation here. Because we leave PBS_NODEFILE
# whole (ezpz does the split internally from --nproc), a plain `ezpz yeet`
...
# keeps the active count constant across retries (in-place swap by index),
# so this is valid for the whole run; os.environ["WORLD_SIZE"] also equals it.
...
# (not mounted on Sunspot); the resident data (/flare .../books-dataset) is
# the `books` list, so that is the default here. DFL_NAME stays overridable.
...
#   blendcorpus (default)                  -> BlendCorpus with $DFL data-list
#   <hf-org>/<repo> e.g. allenai/olmo-mix-1124 -> HuggingFace streaming
...
# (e.g. a machine without the canonical olmo-mix-1124 data list).
...
# ("--max-failover-retries 3") that argparse rejects.
mfr_args=()
