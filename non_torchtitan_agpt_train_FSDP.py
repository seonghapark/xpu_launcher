#!/usr/bin/env python3
"""FSDP2 aGPT training smoke test for XPU (no TorchTitan).

This is the aGPT counterpart of non_torchtitan_gemma_train_FSDP.py. It loads an
HF-format aGPT checkpoint (a DCP checkpoint converted with
convert_dcp_to_safetensors.py, which resolves to LlamaForCausalLM), builds a
token batch from Hugging Face datasets (or synthetic tokens), and runs
forward/backward/optimizer steps with optional FSDP2 sharding. By default only
lm_head is trainable; pass --train-mode full for full-model training.
"""
...
DEFAULT_MODEL_PATH = "/lus/flare/projects/datascience/seonghapark/agpt-2b-v2-256n-step-92859-safetensors"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="HF-format aGPT model directory (converted from DCP).",
    )
    parser.add_argument(
        "--dataset-path",
        default="synthetic",
    ...
    parser.add_argument("--fsdp-param-dtype", default="bfloat16", choices=("bfloat16", "float16", "float32"))
    parser.add_argument("--fsdp-reduce-dtype", default="float32", choices=("bfloat16", "float16", "float32"))
    parser.add_argument(
        "--disable-fsdp",
        action="store_true",
        help="Disable FSDP2 wrapping for debugging.",
    )
    ...
    parser.add_argument("--wandb-project", default=os.environ.get("WANDB_PROJECT", "xpu-non-torchtitan-agpt"))
...
def apply_fsdp2_sharding(model: Any, torch: Any, dist_info: dict[str, Any], args: argparse.Namespace) -> bool:
    if args.disable_fsdp or dist_info["world_size"] <= 1:
        return False
    ...
    mesh = init_device_mesh(
        ...
        mesh_shape=(dist_info["world_size"],),
        mesh_dim_names=("fsdp",),
    )
    mp_policy = MixedPrecisionPolicy(
        param_dtype=dtype_from_name(torch, args.fsdp_param_dtype),
        reduce_dtype=dtype_from_name(torch, args.fsdp_reduce_dtype),
        cast_forward_inputs=False,
    )
    fsdp_kwargs = {"mesh": mesh["fsdp"], "mp_policy": mp_policy}

    decoder_layers = getattr(getattr(model, "model", None), "layers", None)
    if decoder_layers is None:
        raise RuntimeError("expected Llama-style model.model.layers for FSDP2 sharding")
...
def main() -> int:
    ...
    fsdp_enabled = apply_fsdp2_sharding(model, torch, dist_info, args)
    model.train()
    ...
    summary_path = log_dir / f"non_torchtitan_agpt_train_rank{rank}.json"
